...
import { Primitives, Color, Axis } from "../rendering/types.js";
...
export class Debug {
      ...
      private readonly CELL_DIM = 0.8;
      private readonly END_OF_SCREEN = 1000;
      constructor( private game: GameController ){

      }
      private cameraControllerComponent( camera: Camera ){
            const createSlider = ( label: string, min: number, max: number, defaultVal: number = Math.round((max + min) /2) )=>{
      ...
      drawYZGrid( color: Color = { r: 1, g: 0, b: 0, a: 1 } ): void {
            ...
            for( let i = 0; i < 300; i++ ){
                  gridVertices.push( 
                        0 ,-1, i * this.CELL_DIM - 1,
                        0 , this.END_OF_SCREEN, i * this.CELL_DIM - 1,
                  )
            }
            for( let i = 0; i < 300; i++ ){
                  gridVertices.push( 
                        0 , i * this.CELL_DIM -1, -1,
